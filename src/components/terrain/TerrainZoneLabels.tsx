...
export default React.memo(function TerrainZoneLabels({ kpis, revealedKpis, focusedKpi, hoveredKpi, onFocusKpi, onClickKpi, visible = true }: Props) {
  const zones = useMemo(() => {
      ...
      <div style={{
        ...
        <AnimatePresence>
          {zones.map((z, i) => {
            ...
            const isHovered = hoveredKpi === z.kpi
            ...
            return (
              <motion.button
                ...
                style={{
                  ...
                  background: isActive ? `rgba(${hexToRgb(z.color)},0.15)` : "rgba(255,255,255,0.02)",
                  border: isActive ? `1px solid ${z.color}55` : "1px solid rgba(255,255,255,0.03)",
                  cursor: "pointer",
              ...
              >
                <span style={{
                  fontSize: 18,
                  lineHeight: 1,
                  filter: isActive ? `drop-shadow(0 0 10px ${z.color})` : "none",
                ...
                <span style={{
                  ...
                  color: isActive ? z.color : "rgba(200,220,240,0.65)",
                  letterSpacing: "0.08em",
                  textTransform: "uppercase",
                  whiteSpace: "nowrap",
                  fontFamily: "'Inter', system-ui, sans-serif",
                  transition: "color 0.2s",
                  textShadow: isActive ? `0 0 8px ${z.color}` : "none",
                }}>
                  {z.label.split(" ")[0]}
                </span>
                <div style={{
                  width: isActive ? 30 : 18, height: 3, borderRadius: 2,
                  background: z.color,
                  opacity: isActive ? 1.0 : 0.45,
                  transition: "all 0.2s",
                  boxShadow: isActive ? `0 0 12px ${z.color}, 0 0 4px ${z.color}` : "none",
                }} />
              </motion.button>
            )
          })}
