...
> = {
  base: { growth: 1, runway: 1, burn: 1, risk: 1, value: 1 },
  upside: { growth: 1.3, runway: 1.2, burn: 0.85, risk: 0.7, value: 1.5 },
  downside: { growth: 0.7, runway: 0.8, burn: 1.2, risk: 1.3, value: 0.8 },
  extreme: { growth: 0.4, runway: 0.5, burn: 1.5, risk: 1.7, value: 0.5 },
};
...
export function calculateMetrics(
  ...
  // Cash is affected by growth, burn, opex, wages
  const cash =
    baseline.cash *
  ...
  // Runway: cash ($m) / burn ($k per month) => months
  const cashInK = cash * 1000;
  const runway = burn > 0 ? Math.max(0, Math.min(36, cashInK / burn)) : 36;
...
const METRIC_RANGES: Record<MetricId, { min: number; max: number }> = {
  runway: { min: 0, max: 36 },
  cash: { min: 0, max: 10 },
