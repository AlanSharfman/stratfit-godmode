...
import type {
    RiskWeatherState as VisualWeatherState,
    WeatherCondition as VisualWeatherCondition,
    WeatherAlert as VisualWeatherAlert,
    WeatherForecast as VisualWeatherForecast,
...
/**
 * Risk Weather Store — unified domain store for all weather/risk state.
 *
 * Merges zone-level risk assessment with visual weather rendering state.
 */
type RiskWeatherStoreState = {
    // ── Zone-level risk assessment ──
    zones: RiskZone[];
    forecasts: WeatherForecast[];
    globalCondition: WeatherCondition;
    lastUpdated: number;

    // ── Visual weather rendering state (consumed by RiskWeatherSystem) ──
    visualWeather: VisualWeatherState;
    visualEnabled: boolean;
    visualSensitivity: number; // 0-1, how reactive to metric changes
    ...
    // ── Visual weather actions ──
    setVisualWeather: (weather: Partial<VisualWeatherState>) => void;
    setVisualEnabled: (enabled: boolean) => void;
    setVisualSensitivity: (sensitivity: number) => void;
};

const defaultVisualWeather: VisualWeatherState = {
    condition: "clear",
    intensity: 0.2,
    windDirection: [1, 0, 0],
    windSpeed: 0.1,
...
export const useRiskWeatherStore = create<RiskWeatherStoreState>((set, get) => ({
    zones: [],
    forecasts: [],
    globalCondition: "clear",
    lastUpdated: Date.now(),

    // Visual weather rendering state
    visualWeather: defaultVisualWeather,
    visualEnabled: true,
    ...
    // ── Visual weather actions ──
    setVisualWeather: (weather) =>
        set((state) => ({
            visualWeather: { ...state.visualWeather, ...weather },
        })),
    setVisualEnabled: (enabled) => set({ visualEnabled: enabled }),
    setVisualSensitivity: (sensitivity) => set({ visualSensitivity: sensitivity }),
