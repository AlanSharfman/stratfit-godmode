...
/**
 * Creates a 1D DataTexture from a Float32Array of risk values.
 * Values should be in [0..1] range. Stored in R channel.
 * Texture width = values.length, height = 1.
 */
export function createRiskTexture(values: Float32Array): THREE.DataTexture {
    ...
    for (let i = 0; i < width; i++) {
        ...
        data[i * 4 + 0] = byte; // R — risk intensity
